...
pub struct QueryEnv {
    ...
    // Error sink held outside the shared inner so a subscription can give each payload its own sink
    // (via `fork_errors`) when items resolve concurrently, while still sharing data/loaders.
...
impl QueryEnv {
    ...
    /// Return a clone that shares the same request data, schema, extensions, and operation, but
    /// resolves errors into its own fresh sink. Used so concurrently-resolved subscription payloads
    /// don't cross-contaminate each other's errors.
    #[doc(hidden)]
