...
pub async fn test_subscription_resolution_concurrency() {
    ...
    impl Item {
        async fn value(&self, ctx: &Context<'_>) -> usize {
            // Every payload blocks here until all COUNT have arrived. The barrier can only release
            // if the payloads resolve concurrently; under serial resolution the first payload would
            // wait here forever.
    ...
    // Serial resolution would never release the barrier and hang; the timeout turns that into a
    // clean failure instead of a stuck test. (Default serial resolution is covered by the other
    // subscription tests, which all run at the default concurrency of 1.)
    ...
    // The barrier released, so all COUNT resolved at once, and `buffered` kept the output in
    // submission order.
...
pub async fn test_subscription_resolution_error_isolation() {
    ...
    impl Item {
        async fn value(&self, ctx: &Context<'_>) -> usize {
            // Record this item's error, then wait until every payload has recorded its own. This
            // holds all COUNT payloads in flight together with their errors in the sink, so if the
            // sink were shared the first payload to drain would scoop up everyone's errors. With
            // `fork_errors` each payload drains only its own sink. (The barrier also requires the
    ...
    // Payload `i` must carry item `i`'s data *and* exactly item `i`'s own error, never a sibling's.
